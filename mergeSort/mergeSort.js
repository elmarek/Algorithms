...
var mergeSort = function(array) {
  ...
  // recursively break down the halves
  var firstMerge = mergeSort(first)
  // recursively break down the halves
  var secondMerge = mergeSort(second)


 var result = [];
// iterate over the sorted arrays and push values to result array
 var merger = function (firstMerge, secondMerge) {

    while (firstMerge.length > 0 && secondMerge.length > 0) {
        if (firstMerge[0] > secondMerge[0]) {
            result.push(secondMerge[0])
            secondMerge.splice(0, 1)
        } else {
            result.push(firstMerge[0])
            firstMerge.splice(0, 1)
        }
    }

    while (firstMerge.length > 0) {

        result.push(firstMerge[0])
        firstMerge.splice(0, 1)
    }

    while (secondMerge.length > 0) {
        result.push(secondMerge[0])
        secondMerge.splice(0, 1)
    }
...
// complexity of merge sort: Logarithmic